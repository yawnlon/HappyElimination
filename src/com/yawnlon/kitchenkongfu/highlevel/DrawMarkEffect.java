...
public class DrawMarkEffect {
	...
	private void initVertexBuffer(int col, int row) {
		...
		{ -w + deltaX, h + deltaY, 0, -w + deltaX, -h + deltaY, 0, w + deltaX, -h + deltaY, 0, w + deltaX, -h + deltaY,
				0, w + deltaX, h + deltaY, 0, -w + deltaX, h + deltaY, 0 };
		// 创建顶点坐标数据缓冲
	...
	private void initTextureBuffer(int witch) {
		...
		{ (witch - 1) * textureRatio, 0, (witch - 1) * textureRatio, 1, witch * textureRatio, 1, witch * textureRatio,
				1, witch * textureRatio, 0, (witch - 1) * textureRatio, 0 };
	...
	public void draw(GL10 gl, int col, int row) {
		...
		// gl.glTranslatef(col * textureRatio, row * textureRatio, 0);
		// //在x=col,y=row的位置绘制选定的素材对象
		// 顶点坐标，允许使用顶点数组
