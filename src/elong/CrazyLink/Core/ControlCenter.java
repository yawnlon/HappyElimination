...
public class ControlCenter {
	...
	static void markDisappear(int token) {
		markDisappear(token, -1, -1, -1, -1);
	}

	// 将成行的动物标记出来
	static void markDisappear(int token, int col1, int row1, int col2, int row2) {
		if (-1 == token)
			return;
		int markCount = 0;
		for (int i = 0; i < (int) CrazyLinkConstent.GRID_NUM; i++) {
	...
	public static boolean isMonster(int col, int row) {
		if (ANIMAL_MONSTER == mAnimalPic[col][row])
			return true;
		else
			return false;
	}
	...
	public static boolean isBomb(int col, int row) {
		if (ANIMAL_BOMB == mAnimalPic[col][row])
			return true;
		else
			return false;
	}
	...
	public static boolean isLaser(int col, int row) {
		if (ANIMAL_LASER == mAnimalPic[col][row])
			return true;
		else
			return false;
	}
	...
	void drawDisappeareRun(GL10 gl, int col, int row) {
		...
		if (drawDisappear != null) {
			...
			if (ctl.isRun())
				drawDisappear.draw(gl, getPicId(col, row), col, row);
		}
	}
